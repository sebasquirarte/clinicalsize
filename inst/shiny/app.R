  ...
  div(
    ...
    div(
      ...
      conditionalPanel(
        ...
        numericInput("k", "Allocation Ratio (k):",
...
server <- function(input, output, session) {
  ...
  results <- reactive({
    ...
    tryCatch(
      do.call(sample_size, c(common_params(), list(
        beta = input$beta, x1 = input$x1
      ))),
  ...
  range_result <- reactive({
    if (!calculated()) return(NULL)
    tryCatch(
      do.call(sample_size_range, c(common_params(), list(
        x1_range = c(input$x1_min, input$x1_max), step = input$step
      ))),
          ...
          tags$td(
            ...
            tags$span(
              style = paste0(
                ...
                "background-color:", bg, "; color:", txt, "; font-weight: 700; font-size: 12px;"
              ),
              paste0(val, "%")
            )
          )
        } else {
          tags$td(style = "padding: 8px 14px; text-align: center; color: #334155;", format(val))
        }
      })
      tags$tr(
        style = paste0(
